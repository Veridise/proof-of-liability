import { Bool, Field, MerkleWitness, Poseidon, PrivateKey, Provable, PublicKey, Reducer, Signature, SmartContract, State, Struct, method, state } from "o1js";

const height = 32;
export class LiabilityWitness extends MerkleWitness(height) {}

class LiabilityLeaf extends Struct({account: PublicKey, balance: Field, prev: Field}) {
    toFields(): Field[] {
...
class LiabilityProof extends Struct({leaf: LiabilityLeaf, witness: LiabilityWitness}) {
...
export class Swap extends Struct({account: PublicKey, fromAmount: Field, toAmount: Field, from: Field, toId: Field, fromId: Field, prevFrom: Field, prevTo: Field}) {
    toFields(): Field[] {
        return [this.fromAmount, this.toAmount, this.from, this.toId, this.fromId, this.prevFrom, this.prevTo].concat(this.account.toFields())
    }
...
export class LiabilityTree extends SmartContract {
    ...
    @method swapFrom(key: PrivateKey, req: Swap, sig: Signature, proof: LiabilityProof) {
        ...
        key.toPublicKey().assertEquals(exchange);
        sig.verify(req.account, req.toFields()).assertEquals(Bool(true))
        req.prevFrom.assertEquals(proof.leaf.hash())
        req.account.assertEquals(proof.leaf.account)
        proof.witness.calculateRoot(req.prevFrom).assertEquals(root);
        req.fromId.assertEquals(tid);

        proof.leaf.balance.assertGreaterThanOrEqual(req.fromAmount);

        let newBal = proof.leaf.balance.sub(req.fromAmount);
        let newLeaf:LiabilityLeaf = new LiabilityLeaf({account: req.account, balance: newBal, prev: req.prevFrom});
        let newRoot = proof.witness.calculateRoot(newLeaf.hash());
    ...
    @method swapTo(key: PrivateKey, req: Swap, sig: Signature, proof: LiabilityProof) {
        ...
        key.toPublicKey().assertEquals(exchange);
        sig.verify(req.account, req.toFields()).assertEquals(Bool(true));
        req.prevTo.equals(0).or(req.prevTo.equals(proof.leaf.hash())).assertTrue();
        req.prevTo.equals(0).or(req.account.equals(proof.leaf.account)).assertTrue();
        proof.witness.calculateRoot(req.prevTo).assertEquals(root);
        req.toId.assertEquals(tid);

        let oldBal = Provable.if(req.prevTo.equals(0), Field, Field(0), proof.leaf.balance);

        let newBal = oldBal.add(req.toAmount);
        newBal.assertGreaterThanOrEqual(oldBal);
        newBal.assertGreaterThanOrEqual(req.toAmount);
        let newLeaf:LiabilityLeaf = new LiabilityLeaf({account: req.account, balance: newBal, prev: req.prevTo});
        let newRoot = proof.witness.calculateRoot(newLeaf.hash());
