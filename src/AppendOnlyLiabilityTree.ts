import { Bool, Field, MerkleWitness, Poseidon, PrivateKey, PublicKey, Reducer, Signature, SmartContract, State, Struct, method, state } from "o1js";

const height = 32;
export class AppendLiabilityWitness extends MerkleWitness(height) {}
export class Deposit extends Struct({balance: Field, account: PublicKey, nonce: Field, prev: Field, ind: Field}) {
    toFields(): Field[] {
        return [this.balance, this.nonce, this.prev].concat(this.account.toFields())
    }
...
export class Withdraw extends Struct({balance: Field, account: PublicKey, nonce: Field, prev: Field, ind: Field}) {
    toFields(): Field[] {
        return [this.balance, this.nonce, this.prev].concat(this.account.toFields())
    }
...
export class Swap extends Struct({balance: Field, account: PublicKey, nonce: Field, from: Field, to: Field, prev: Field, ind: Field}) {
    toFields(): Field[] {
        return [this.balance, this.nonce, this.prev, this.from, this.to].concat(this.account.toFields())
    }
...
export class AppendOnlyLiabilityTree extends SmartContract {
    zeros = [
        Field(0n),
    ...
    @method deposit(key: PrivateKey, req: Deposit, sig: Signature, newWitness: AppendLiabilityWitness, prevWitness: AppendLiabilityWitness) {
        ...
        ind.assertEquals(req.ind);

        sig.verify(req.account, req.toFields()).assertEquals(Bool(true));
        key.toPublicKey().assertEquals(exchange);

        prevWitness.calculateIndex().assertLessThan(ind);
        prevWitness.calculateRoot(req.prev).assertEquals(root);

        newWitness.calculateIndex().assertEquals(ind);
        newWitness.calculateRoot(this.zero(0)).assertEquals(root);
        const newRoot = newWitness.calculateRoot(req.hash());
        this.root.set(newRoot);
        this.ind.set(ind.add(1));
    }

    @method withdraw(key: PrivateKey, req: Withdraw, sig: Signature, newWitness: AppendLiabilityWitness, prevWitness: AppendLiabilityWitness) {
        let exchange = this.exchange.get();
        this.exchange.assertEquals(exchange);
    ...
    @method swap(key: PrivateKey, req: Swap, sig: Signature, newWitness: AppendLiabilityWitness, prevWitness: AppendLiabilityWitness) {
        let exchange = this.exchange.get();
        this.exchange.assertEquals(exchange);
